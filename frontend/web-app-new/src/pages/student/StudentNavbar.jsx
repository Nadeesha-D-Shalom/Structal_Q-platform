...
const StudentNavbar = () => {
  ...
  const getActivePage = () => {
    ...
    if (path.startsWith("/student/submissions")) return "Submissions";
    if (path.startsWith("/student/marks")) return "Grades & Marks";
    if (
      path.startsWith("/student/timetable") ||
      path.startsWith("/student-view") ||
      path === "/view"
    ) {
      return "Timetable";
    }
    if (
      path.startsWith("/student/concerns") ||
      path.startsWith("/student/raise-concern") ||
      path.startsWith("/raise-concern")
    ) {
      return "Concerns";
    }
    return "Dashboard";
